...
const server = http.createServer((request, response) => {
    ...
    if (url == '/') {
        ...
        let reply = 'No argument provided.\nUsage: node pets.js [read | create | update | destroy]';
        ...
        fs.readFile('pets.json', 'utf8', function (error, data) {
            ...
            if (error) {
                ...
                //console.error(new Error(error));
                let reply = "Non-existant or incorrect index. \n Usage: node pets.js read INDEX";
                response.setHeader('Content-Type', 'application/json');
                response.statusCode = 404;
                response.end(JSON.stringify(reply));
                //process.exitCode = 1;
        ...
        if (age == undefined || name == undefined || kind == undefined) {
            console.error(new Error("Did not provide required information \n Usage: node pets.js create AGE KIND NAME"))
            exit(1);
        }
        else {
            fs.readFile('pets.json', 'utf8', function (error, data) {
        ...
        console.error(new Error('Bad argument provided.\nUsage: node pets.js [read | create | update | destroy]'));
        process.exitCode = 1;
    }
});
